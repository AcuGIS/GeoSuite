...
function install_pg_tileserv(){
  ...
  pushd ${TILESERV_HOME}
    wget --quiet -P/opt https://postgisftw.s3.amazonaws.com/pg_tileserv_latest_linux.zip
    unzip /opt/pg_tileserv_latest_linux.zip
    rm -f /opt/pg_tileserv_latest_linux.zip
...
function install_pg_featureserv(){
  ...
  pushd ${FEATSERV_HOME}
    wget --quiet -P/opt https://postgisftw.s3.amazonaws.com/pg_featureserv_latest_linux.zip
    unzip /opt/pg_featureserv_latest_linux.zip
    rm -f /opt/pg_featureserv_latest_linux.zip
...
function install_bootstrap_app(){
	wget --quiet -P/opt https://github.com/AcuGIS/GeoHelm/archive/refs/heads/master.zip
	unzip /opt/master.zip -d/opt

	cp -r /opt/GeoHelm-master/app/* /var/www/html/
	mv /opt/GeoHelm-master/app/data /opt/
...
function install_openlayers(){
  ...
  wget --quiet -P/opt "https://github.com/openlayers/openlayers/releases/download/v${OL_VER}/v${OL_VER}-package.zip"

	mkdir /var/www/html/OpenLayers
	pushd /var/www/html/OpenLayers
	  unzip -u /opt/v${OL_VER}-package.zip
	popd
	rm -f /opt/v${OL_VER}-package.zip
...
function install_leafletjs(){
  ...
  wget --quiet -P/opt "https://leafletjs-cdn.s3.amazonaws.com/content/leaflet/v${LL_VER}/leaflet.zip"

  unzip /opt/leaflet.zip -d /var/www/html/leafletjs
  rm -f /opt/leaflet.zip
...
function install_tomcat(){
	...
	if [ ! -d apache-tomcat-${TOMCAT_VER} ]; then
		if [ ! -f /opt/apache-tomcat-${TOMCAT_VER}.tar.gz ]; then
			wget -P/opt http://www.apache.org/dist/tomcat/tomcat-${TOMCAT_MAJOR}/v${TOMCAT_VER}/bin/apache-tomcat-${TOMCAT_VER}.tar.gz
		fi
		tar xzf /opt/apache-tomcat-${TOMCAT_VER}.tar.gz
		chown -R tomcat:tomcat apache-tomcat-${TOMCAT_VER}
		rm -rf /opt/apache-tomcat-${TOMCAT_VER}.tar.gz
	fi
...
function install_geoserver(){
	...
	if [ ! -f /opt/geoserver-${GEO_VER}-war.zip ]; then
		wget -P/opt http://sourceforge.net/projects/geoserver/files/GeoServer/${GEO_VER}/geoserver-${GEO_VER}-war.zip
	fi

	unzip -ou /opt/geoserver-${GEO_VER}-war.zip -d/opt/
	mv /opt/geoserver.war ${CATALINA_HOME}/webapps/
	chown -R tomcat:tomcat ${CATALINA_HOME}/webapps/geoserver.war
	rm -f /opt/geoserver-${GEO_VER}-war.zip
...
function whiptail_gauge(){
  ...
  for step in "${!STEPS[@]}"; do
    ...
    ${CMDS[$step]} 1>"/opt/${CMDS[$step]}.log" 2>&1
...
function provision_ssl(){
	/bin/bash /opt/build-ssl.sh || true
}
