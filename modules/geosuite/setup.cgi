...
sub install_bootstrap_web_app(){
	...
	my $url = "https://cdn.acugis.com/geosuite/docs.tar.bz2";
	$progress_callback_url = $url;
	&error_setup(&text('install_err3', $url));
	my $tmpfile = &transname("docs.tar.bz2");
	&http_download('cdn.acugis.com', 443, "/geosuite/docs.tar.bz2", $tmpfile, \$error, \&progress_callback, 1);
...
sub setup_apache_for_geoserver(){
	...
	if(	( $osinfo{'real_os_type'} =~ /centos/i) or	#CentOS
		...
		$gs_proxy_file = '/etc/httpd/conf.d/geoserver.conf';
		...
		$gs_proxy_file = '/etc/apache2/conf-enabled/geoserver.conf';
	}
...
sub select_tomcat_archive{
	...
	function update_select(){
		...
		get_pjax_content('/geosuite/setup.cgi?mode=tomcat_install_form&tmver='+major);
	}
	</script>
EOF
...
sub setup_checks{
	...
	if (!&has_command('java')) {
		print '<p>Warning: Java is not found. Install it manually or from the '.
			  "<a href='./edit_java.cgi?return=%2E%2E%2Fgeosuite%2Fsetup.cgi&returndesc=Setup&caller=geosuite'>Java tab</a></p>";
	}
	...
	if(scalar(@dep_pkgs) > 0){
		...
		print "<p>Warning: $pkg_list packages are not installed. Install them manually or ".
				"<a href='../software/install_pack.cgi?source=3&update=".$pkgs_urlize."&return=%2E%2E%2Fgeosuite%2Fsetup.cgi&returndesc=Setup&caller=geosuite'>click here</a> to have them installed.</p>";
	}

	my @pinfo = software::package_info('haveged', undef, );
	if(!@pinfo){
		print "<p>Warning: haveged package is not installed. Install it manually or ".
			  "<a href='../software/install_pack.cgi?source=3&update=haveged&return=%2E%2E%2Fgeosuite%2Fsetup.cgi&returndesc=Setup&caller=geosuite'>click here</a> to have it downloaded and installed.</p>";
	}
	...
	if (! -f "$module_config_directory/bootstraped.txt"){
		print '<p>Warning: Bootstrap web app is not installed in /var/www/html. '.
			  "<a href='setup.cgi?mode=install_bootstrap_web_app&return=%2E%2E%2Fgeosuite%2Fsetup.cgi&returndesc=Setup&caller=geosuite'>Click here</a> install it";
	}

	# Check if OpenLayers exists
	if (! -d "/var/www/html/OpenLayers"){
		print "<p>The OpenLayers direcrory <tt>/var/www/html/OpenLayers</tt> does not exist. ".
			  "<a href='setup.cgi?mode=install_openlayers&return=%2E%2E%2Fgeosuite%2Fsetup.cgi&returndesc=Setup&caller=geosuite'>Click here</a> install it";
	}

	# Check if LeafletJS exists
	if (! -d "/var/www/html/leafletjs"){
		print "<p>The LeafletJS direcrory <tt>/var/www/html/leafletjs</tt> does not exist. ".
			  "<a href='setup.cgi?mode=install_leafletjs&return=%2E%2E%2Fgeosuite%2Fsetup.cgi&returndesc=Setup&caller=geosuite'>Click here</a> install it";
	}
		...
		if (! -d "$catalina_home/webapps/geoserver/") {
			if( -f "$catalina_home/webapps/geoserver.war"){
				...
				print "<p>The Geoserver webapp direcrory <tt>$catalina_home/webapps/geoserver/</tt> does not exist. ".
					  "<a href='./install_war.cgi?source=2&url=$url&return=%2E%2E%2Fgeosuite%2Fsetup.cgi&returndesc=Setup&caller=geosuite'>Click here</a> to have it downloaded and installed.</p>";
			}
		}
	}
	...
	if(! -f $gs_proxy_file){
		print "<p>The GeoServer Apache config <tt> $gs_proxy_file</tt> does not exist. ".
			  "<a href='./setup.cgi?mode=setup_geoserver_apache&return=%2E%2E%2Fgeosuite%2Fsetup.cgi&returndesc=Setup&caller=geosuite'>Click here</a> to create it.";
	}
			...
			if (!&has_command('shp2pgsql')) {
				my $shp2pg_pkg = get_shp2pgsql_pkg_name($pg_ver);
					print '<p>Warning: shp2pgsql command is not found. '.
					  "<a href='../package-updates/update.cgi?mode=new&source=3&u=$shp2pg_pkg&redir=%2E%2E%2Fgeosuite%2Fsetup.cgi&redirdesc=Setup'>Click here</a> to have it installed from $shp2pg_pkg package.</p>";
			}
			...
			foreach my $pkg (@ext_pkgs){
				...
				if(!@pinfo){
					print "<p>Warning: $pkg package is not installed. Install it manually or ".
						  "<a href='../software/install_pack.cgi?source=3&update=$pkg&return=%2E%2E%2Fgeosuite%2Fsetup.cgi&returndesc=Setup&caller=geosuite'>click here</a> to have it downloaded and installed.</p>";
				}
			}
		}
	}
	...
	print '<p>If you don\'t see any warnings above, you can complete setup by clicking '.
		  "<a href='setup.cgi?mode=cleanup&return=%2E%2E%2Fgeosuite%2F&returndesc=Geohelm&caller=geosuite'>here</a></p>";
}
